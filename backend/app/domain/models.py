"""Modelos SQLAlchemy que representam o domínio do AtlasRAG."""
...
# Dimensão padrão de embeddings conforme modelo base.
EMBEDDING_DIM = 1536
...
class Scan(Base):
    """Registro de execução de varreduras."""
    __tablename__ = "scans"
...
class DbSchema(Base):
    """Schema identificado durante a varredura."""
...
class DbTable(Base):
    """Tabela do banco de dados com anotações."""
...
class DbColumn(Base):
    """Coluna de tabela com metadados."""
...
class DbConstraint(Base):
    """Constraint de tabela (PK, FK, etc.)."""
...
class DbIndex(Base):
    """Índice de tabela capturado na varredura."""
...
class Sample(Base):
    """Amostras de linhas para contextos de RAG."""
...
class ApiRouteField(Base):
    """Campos esperados das rotas de API."""
...
class Embedding(Base):
    """Embeddings persistidos para consulta semântica."""
...
class Agent(Base):
    """Agente configurável para interação RAG."""
