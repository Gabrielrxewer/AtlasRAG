"""Endpoints para gerenciar agentes e conversas."""
import logging
...
from app.application.services.agents import build_agent_reply
...
def _validate_related_ids(db: Session, ids: list[int], model, label: str) -> None:
    """Valida IDs relacionados para evitar referências inválidas."""
    if not ids:
        return
...
def create_agent(payload: AgentCreate, db: Session = Depends(get_db)):
    # Cria agente com validação de conexões e APIs.
...
def update_agent(agent_id: int, payload: AgentUpdate, db: Session = Depends(get_db)):
    # Atualiza agentes e valida relacionamentos.
...
def list_agent_messages(agent_id: int, db: Session = Depends(get_db)):
    # Lista histórico de mensagens do agente.
...
def send_agent_message(agent_id: int, payload: AgentMessageCreate, db: Session = Depends(get_db)):
    # Envia mensagem e registra resposta do agente.
